...
const RouteRedirectIntegration = (): AstroIntegration => ({
	...
	hooks: {
		...
		"astro:build:done": async ({ dir, logger }) => {
			...
			// Generate a targeted set of static redirects for Cloudflare Pages.
			// Only includes lowercase→PascalCase and semantic aliases (not the
			// full case-permutation set). The SW handles everything else.
			// CF Pages limit: 2000 static rules (all plans).

			const RedirectLine: string[] = [
				"# Auto-generated by RouteRedirect integration",
				"# Lowercase paths → PascalCase canonicals",
			];

			for (const [BuiltPath, PascalPath] of Object.entries(
				PascalCaseCanonical,
			)) {
				if (BuiltPath !== PascalPath && PascalPath !== "/") {
					RedirectLine.push(
						`${BuiltPath} ${PascalPath} 301`,
					);
			...
			RedirectLine.push("");
			RedirectLine.push("# Semantic aliases");

			for (const [Alias, Target] of Object.entries(SemanticAlias)) {
				RedirectLine.push(`${Alias} ${Target} 301`);
			...
			logger.info(
				`Wrote _redirects with ${RedirectLine.filter((L) => !L.startsWith("#") && L.trim()).length} redirect rules`,
			);
