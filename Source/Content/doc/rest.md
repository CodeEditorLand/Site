...
Rest is the TypeScript compiler and bundler for Editor.Land. It is built on OXC
(Oxidation Compiler), a Rust-native TypeScript parser and transformer, combined
with esbuild for final bundling.
...
run inside a Go runtime with its own garbage collector.
...
Parsing, transformation, and code emission happen in native code with no garbage
collection pauses.
...
during development are substantially faster than running tsc directly. The exact
improvement depends on the size of the TypeScript surface being compiled.
