...
Wind's core service surface is active and growing with each release. The
Effect-TS Layer infrastructure is in place, consumed by Sky in the
`debug-mountain` profile on macOS and Windows, and the most commonly used
workbench services are confirmed working on both platforms.
...
- **`Preload.ts`** - the preload script injected into the Tauri WebView
  context. It exposes the typed IPC bridge that Sky components use at
  runtime before the full Layer stack is initialized.
...
This is still an IPC mechanism. Calls cross a process boundary and arguments
are serialized. The difference from Electron's IPC is that Tauri's bindings
are typed at the call site (no raw `ipcRenderer.send('event-name', payload)`)
and the receiving end in Mountain is a Rust function with a matching type
signature rather than an untyped event handler.

In-process state: UI panel layout, command palette state, active editor
tabs. This propagates through the Effect runtime without crossing the IPC
...
`Source/Service/TauriMainProcessService.ts` is Wind's central wrapper around
all `invoke` calls to Mountain. Each named Tauri command that Mountain exposes
has a corresponding typed method in this service. Sky components and Wind
layers call methods on this service rather than calling `invoke` with raw
strings. This gives the compiler full visibility into the parameter and return
