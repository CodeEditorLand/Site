...
const DynamicBlogCard = ({ Post }: { Post: BlogPost }) => (
	<article className="StaccatoCard border border-[var(--ColorBorder)] bg-[var(--ColorCard)] p-6">
		{Post.Tags.length > 0 && (
			<div className="mb-3 flex flex-wrap gap-2">
				{Post.Tags.map((Tag) => (
					<span
						key={Tag}
						className="border border-[var(--ColorBorder)] px-2 py-0.5 text-xs font-medium text-muted-foreground">
						{Tag}
					</span>
				))}
			</div>
