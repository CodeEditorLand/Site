import { i as t } from "./index.DSBWf7TZ.js";
const e = "data-astro-transition-persist",
...
const i = ({ activeElement: t, start: e, end: n }) => {
		t &&
			(t.focus(),
	...
	a = (t, n) => {
		const o = t.getAttribute(e),
			r = o && n.head.querySelector(`[${e}="${o}"]`);
		if (r) return r;
		if (t.matches("link[rel=stylesheet]")) {
			const e = t.getAttribute("href");
			return n.head.querySelector(`link[rel=stylesheet][href="${e}"]`);
		}
		return null;
	},
	s = (t) => {
	...
	l = (t, e) => t.getAttribute("props") === e.getAttribute("props"),
	c = (t) => {
		!(function (t) {
			...
			(function (t) {
				const e = document.documentElement,
					o = [...e.attributes].filter(
						({ name: t }) => (e.removeAttribute(t), n.includes(t)),
					);
				[...t.documentElement.attributes, ...o].forEach(
					({ name: t, value: n }) => e.setAttribute(t, n),
				);
			})(t),
			(function (t) {
			...
			if (t?.closest(`[${e}]`)) {
				if (
					...
					const e = t.selectionStart,
						n = t.selectionEnd;
					return () => i({ activeElement: t, start: e, end: n });
				}
				return () => i({ activeElement: t });
			}
			return () => i({ activeElement: null });
		})();
		!(function (t, n) {
			n.replaceWith(t);
			for (const o of n.querySelectorAll(`[${e}]`)) {
				const n = o.getAttribute(e),
					r = t.querySelector(`[${e}="${n}"]`);
				r &&
					(r.replaceWith(o),
					"astro-island" === r.localName &&
						s(o) &&
						!l(o, r) &&
						(o.setAttribute("ssr", ""),
						o.setAttribute("props", r.getAttribute("props"))));
			}
...
class u extends Event {
	...
	newDocument;
	signal;
	constructor(t, e, n, o, r, i, a, s, l, c) {
		super(t, e),
			(this.from = n),
			(this.to = o),
			(this.direction = r),
...
async function f(t, e, n, o, r, i, a, s, l) {
	const c = new d(t, e, n, o, r, i, window.document, a, s, l);
	var u;
	return (
		document.dispatchEvent(c) &&
			(await c.loader(),
			c.defaultPrevented ||
				((u = "astro:after-preparation"),
				document.dispatchEvent(new Event(u)),
				"traverse" !== c.navigationType &&
					v({ scrollX: scrollX, scrollY: scrollY }))),
		c
	);
}
const h = history.pushState.bind(history),
	p = history.replaceState.bind(history),
	v = (t) => {
		history.state &&
	...
	w = (t, e) => t.pathname === e.pathname && t.search === e.search;
let g, E, T;
const A = (t) => document.dispatchEvent(new Event(t)),
	S = () => A("astro:page-load"),
...
history.state
	? ((P = history.state.index),
		scrollTo({ left: history.state.scrollX, top: history.state.scrollY }))
	: y() &&
		(p({ index: P, scrollX: scrollX, scrollY: scrollY }, ""),
		(history.scrollRestoration = "manual"));
const Y = (t, e, n, o, r) => {
	const i = w(e, t),
		a = document.title;
	...
	if (t.href !== location.href && !r)
		if ("replace" === n.history) {
			...
			h({ ...n.state, index: ++P, scrollX: 0, scrollY: 0 }, "", t.href);
	if (
		((document.title = a),
		(T = t),
...
async function R(t, e, n, o, r) {
	async function i(t) {
		const e = document.getAnimations();
		document.documentElement.setAttribute(x, t);
...
async function M(t, e, n, o, r) {
	const i =
		(g?.controller.abort(), (g = { controller: new AbortController() }));
	if (!y() || location.origin !== n.origin)
		return i === g && (g = void 0), void (location.href = n.href);
	const a = r ? "traverse" : "replace" === o.history ? "replace" : "push";
	if (
		("traverse" !== a && v({ scrollX: scrollX, scrollY: scrollY }),
		w(e, n) && (("back" !== t && n.hash) || ("back" === t && e.hash)))
	)
		return Y(n, e, o, document.title, r), void (i === g && (g = void 0));
	const s = await f(
		e,
		n,
		t,
		...
		async function (t) {
			...
			if (null === r) return void t.preventDefault();
			if (r.redirected) {
				const e = new URL(r.redirected);
				if (e.origin !== t.to.origin) return void t.preventDefault();
				t.to = e;
			}
			if (
				((k ??= new DOMParser()),
				(t.newDocument = k.parseFromString(r.html, r.mediaType)),
				t.newDocument
			...
			i.length && !t.signal.aborted && (await Promise.all(i));
		},
	);
	...
	const l = await (async function () {
		...
		return (E = { transitionSkipped: !1 });
	})();
	if (s.signal.aborted) i === g && (g = void 0);
	else {
		if ((document.documentElement.setAttribute(D, s.direction), b))
			l.viewTransition = document.startViewTransition(
				async () => await R(s, o, l, r),
			);
		else {
			const t = (async () => {
				await Promise.resolve(), await R(s, o, l, r, X());
			})();
			l.viewTransition = {
				updateCallbackDone: t,
				ready: t,
				finished: new Promise((t) => (l.viewTransitionFinished = t)),
				skipTransition: () => {
		...
		l.viewTransition?.updateCallbackDone.finally(async () => {
			await (function () {
				...
				for (const n of document.getElementsByTagName("script"))
					void 0 === n.dataset.astroExec &&
						"module" === n.getAttribute("type") &&
						(e = null === n.getAttribute("src"));
				e &&
				...
				for (const n of document.getElementsByTagName("script")) {
					...
					if (e && "module" !== e && "text/javascript" !== e)
						continue;
					...
					(o.dataset.astroExec = ""), n.replaceWith(o);
				}
				return t;
			})(),
				S(),
...
async function H(t, e) {
	await M("forward", T, new URL(t, location.href), e ?? {});
}
const q = () => {
	!history.state ||
...
if (b || "none" !== X())
	if (
		...
		addEventListener("popstate", function (t) {
			if (!y() && t.state) return void location.reload();
			if (null === t.state) return;
			const e = history.state,
				n = e.index,
				o = n > P ? "forward" : "back";
			(P = n), M(o, T, new URL(location.href), {}, e);
		}),
		addEventListener("load", S),
		"onscrollend" in window)
	)
		addEventListener("scrollend", q);
	else {
		let t, e, n, o;
		const r = () =>
			o !== history.state?.index
				...
				: e === scrollY && n === scrollX
					? (clearInterval(t), (t = void 0), void q())
...
for (const $ of document.getElementsByTagName("script"))
	r($), ($.dataset.astroExec = "");
let C = null;
function U(t) {
...
(b ||
	...
	(document.addEventListener("click", (t) => {
		...
		U(e) ||
			...
			H(o, {
				history:
					"replace" === e.dataset.astroHistory ? "replace" : "auto",
				sourceElement: e,
			}));
	}),
	document.addEventListener("submit", (t) => {
		...
		if (
			((C = null),
			"FORM" !== e.tagName || t.defaultPrevented || U(e) || o)
		)
			return;
		const r = e,
			i = new FormData(r, n),
		...
		const c = n?.getAttribute("formmethod") ?? s ?? "get";
		if (
			"dialog" === c ||
			location.origin !== new URL(l, location.href).origin
		)
			return;
		...
		if ("get" === c) {
			const t = new URLSearchParams(i),
				e = new URL(l);
			(e.search = t.toString()), (l = e.toString());
		} else u.formData = i;
		t.preventDefault(), H(l, u);
	}),
	t({ prefetchAll: !0 }));
