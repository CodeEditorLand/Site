//#region Source/Function/Noise/Attention.ts
var Attention_default = (async () => {
	...
	*/
	const Scatter = (Index, SpreadX, SpreadY = 0) => {
		const NoiseX = LayoutNoise(Index * .5, 0);
		const NoiseY = LayoutNoise(0, Index * .5);
		return {
			X: NoiseX * SpreadX,
			Y: NoiseY * SpreadY
		};
	...
	*/
	const ScatterRotation = (Index, MaxDegree = 3) => {
		return LayoutNoise(Index * .37, Index * .91) * MaxDegree;
	};
	/**
	* ScatterScale:returns a noise-driven scale factor.
	* Centers around 1.0, deviates by Spread.
	*/
	const ScatterScale = (Index, Spread = .05) => {
		return 1 + LayoutNoise(Index * .61, Index * .43) * Spread;
	};
	/**
	...
	*/
	const ScatterDelay = (Index, MaxDelay = 200) => {
		const Raw = LayoutNoise(Index * .83, Index * .29);
		return Math.abs(Raw) * MaxDelay;
	};
	/**
	* ScatterOpacity:returns noise-driven opacity between Min and Max.
	*/
	const ScatterOpacity = (Index, Min = .85, Max = 1) => {
		return Min + (LayoutNoise(Index * .47, Index * .67) + 1) / 2 * (Max - Min);
	};
	/**
	...
	*/
	const ObserveAndApply = (Selector, SpreadX, SpreadY = 0, Threshold = .1) => {
		const ElementList = document.querySelectorAll(Selector);
		const Observer = new IntersectionObserver((EntryList) => {
			for (const Entry of EntryList) if (Entry.isIntersecting) {
				const Element = Entry.target;
				const Index = Array.from(ElementList).indexOf(Element);
				if (Index >= 0) ApplyToElement(Element, Index, SpreadX, SpreadY);
	...
	*/
	const InitHoverState = (Element, Config = {}) => {
		...
		const State = {
			Element,
			OriginalTransform: ComputedStyle.transform || "none",
	...
	*/
	const AnimateHoverStates = () => {
		...
		if (HasActiveAnimations) AnimationFrameId = requestAnimationFrame(AnimateHoverStates);
	};
	/**
	...
	*/
	const OnMouseEnter = (Event) => {
		...
		if (!State) State = InitHoverState(Element);
		...
		if (!AnimationFrameId) AnimationFrameId = requestAnimationFrame(AnimateHoverStates);
	};
	/**
...
//#endregion
export { Attention_default as default };
