//#region Source/Function/OpenGraph.ts
/**
...
*/
var WrapText = (Text, MaxCharacter) => {
	const Word = Text.split(" ");
	const Line = [];
	let Current = "";
	for (const W of Word) if (Current.length + W.length + 1 > MaxCharacter && Current.length > 0) {
		Line.push(Current);
		Current = W;
...
var EscapeXML = (Text) => Text.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/"/g, "&quot;").replace(/'/g, "&apos;");
/**
...
*/
var LogoGlyph = () => `<rect width="1005.28" height="620.771" rx="91.3889"
...
*/
var GenerateOpenGraphSvg = (Title, Description, Section) => {
	const Width = 1200;
	const Height = 675;
	...
	const TitleElement = ClampedTitleLine.map((Line, Index) => `<text x="72" y="${TitleStartY + Index * TitleLineHeight}" font-family="system-ui, -apple-system, 'Segoe UI', sans-serif" font-size="${TitleFontSize}" font-weight="700" fill="#151515">${EscapeXML(Line)}</text>`).join("\n		");
	const DescriptionStartY = TitleStartY + ClampedTitleLine.length * TitleLineHeight + 20;
	const DescriptionElement = ClampedDescriptionLine.map((Line, Index) => `<text x="72" y="${DescriptionStartY + Index * DescriptionLineHeight}" font-family="system-ui, -apple-system, 'Segoe UI', sans-serif" font-size="${DescriptionFontSize}" font-weight="400" fill="#666666">${EscapeXML(Line)}</text>`).join("\n		");
...
//#endregion
export { GenerateOpenGraphSvg as default };
