...
const BuildEmailBody = (Config, Values, Year2) => {
  const Code = `[${Config.Code}-${Year2}]`;
  const Lines = [
    Config.BodyPreamble,
    "",
    `Income Code: ${Code}`,
...
const BuildMailtoHref = (Config, Values, Year2) => {
  const Subject = `[${Config.Code}-${Year2}] ${Config.Title} Request${Values["email"] ? ` - ${Values["email"]}` : ""}`;
  const Body = BuildEmailBody(Config, Values, Year2);
...
export { BuildEmailBody as B, Requests as R, BuildMailtoHref as a };
