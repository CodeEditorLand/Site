...
export { z };
import { x as renderElement, s as generateCspDigest, C as spreadAttributes, D as unescapeHTML, B as renderTemplate, A as AstroError, L as LiveContentConfigError, a as AstroUserError, k as RenderUndefinedEntryError, U as UnknownContentCollectionError, G as GetEntryDeprecationError, p as createHeadAndContent, w as renderComponent } from './prerender_DG8BZEWD.mjs';
...
import { V as VALID_INPUT_FORMATS } from './OpenGraph_m_taqD86.mjs';
import 'piccolore';
...
function createGetCollection({
  ...
  return async function getCollection(collection, filter) {
    ...
    if (store.hasCollection(collection)) {
      const { default: imageAssetMap } = await import('./Turbulence_qp3DVTah.mjs').then(n => n.d);
      const result = [];
...
async function updateImageReferencesInBody(html, fileName) {
  ...
  const { getImage } = await import('./_virtual_astro_get-image_DRstbZjF.mjs');
...
async function renderEntry(entry) {
  ...
  if (entry.deferredRender) {
    try {
      const { default: contentModules } = await import('./OpenGraph_m_taqD86.mjs').then(n => n.d);
