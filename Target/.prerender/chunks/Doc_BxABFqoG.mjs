...
import 'piccolore';
import { r as renderComponent, a as renderTemplate, m as maybeRenderHead, b as addAttribute } from './prerender_O3JwF96W.mjs';
import { getCollection } from './_astro_content_Do7_53PI.mjs';
import { D as DynamicDocSidebar } from './DynamicDocSidebar_CoCXJIn0.mjs';
...
const $$Doc = createComponent(async ($$result, $$props, $$slots) => {
  ...
  return renderTemplate`${renderComponent($$result, "Base", $$Base, { "Title": MetaTitle, "Description": MetaDescription, "Url": "https://editor.land/Doc", "lang": "en" }, { "default": async ($$result2) => renderTemplate` ${renderComponent($$result2, "Header", Header, { "client:load": true, "client:component-hydration": "load", "client:component-path": "/Volumes/CORSAIR/Developer/macOS/Application/CodeEditorLand/WebSite/Source/Component/Layout/Header", "client:component-export": "Header" })} ${maybeRenderHead()}<div class="container mx-auto max-w-6xl px-4 py-16"> <div class="flex gap-8"> <!-- Sidebar - sticky on desktop, hidden on mobile --> <aside class="hidden w-56 shrink-0 lg:block"> <div class="sticky top-8"> <p class="mb-3 text-xs font-semibold uppercase tracking-wider text-muted-foreground"> ${T("sidebar.title", {
    ns: "doc",
    defaultValue: "Documentation"
  })} </p> ${renderComponent($$result2, "DynamicDocSidebar", DynamicDocSidebar, { "Sections": SidebarSections, "client:load": true, "client:component-hydration": "load", "client:component-path": "/Volumes/CORSAIR/Developer/macOS/Application/CodeEditorLand/WebSite/Source/Component/Dynamic/DynamicDocSidebar", "client:component-export": "DynamicDocSidebar" })} <div class="mt-4 border-t border-[var(--Border)] pt-4"> <a href="/Doc/Rust" class="block px-3 py-2 text-sm text-muted-foreground transition-colors hover:bg-[var(--Secondary)] focus:outline-2 focus:outline-offset-2 focus:outline-[var(--Primary)]">
Rust API
</a> </div> </div> </aside> <!-- Main content --> <main id="main-content" class="min-w-0 flex-1"> <!-- Breadcrumbs --> <nav aria-label="Breadcrumb" class="mb-8"> <ol class="flex flex-wrap items-center space-x-2 text-sm text-muted-foreground"> <li> <a href="/" class="transition-colors hover:text-foreground">${T("common.breadcrumb.home", {
    defaultValue: "Home"
  })}</a> </li> <li class="mx-2">/</li> <li> <span class="font-medium text-foreground">${T("common.docs.pageTitle", {
    defaultValue: "Documentation"
  })}</span> </li> </ol> </nav> <!-- Page Header --> <header class="mb-12"> <h1 class="mb-4 text-4xl font-bold tracking-tight"> ${T("common.docs.pageTitle", {
    defaultValue: "Documentation"
  })} </h1> <p class="text-lg text-muted-foreground"> ${T("common.docs.pageSubtitle", {
    defaultValue: "Everything you need to install, build, and contribute to Land."
  })} </p> </header> <!-- Quick Links: GitHub, Issues, Rust API --> <div class="grid grid-cols-1 gap-6 md:grid-cols-3"> <a href="https://github.com/CodeEditorLand/Land#readme" target="_blank" rel="noopener noreferrer" class="StaccatoCard StaccatoBorderShimmer block border border-[var(--Border)] bg-white p-6 transition-colors hover:bg-[var(--Secondary)] focus:outline-2 focus:outline-offset-2 focus:outline-[var(--Primary)]"> <h2 class="mb-2 text-xl font-semibold"> ${T("common.docs.gettingStarted.title", {
  ...
  })} </p> </a> <a href="/Doc/Rust" class="StaccatoCard StaccatoBorderShimmer block border border-[var(--Border)] bg-white p-6 transition-colors hover:bg-[var(--Secondary)] focus:outline-2 focus:outline-offset-2 focus:outline-[var(--Primary)]"> <h2 class="mb-2 text-xl font-semibold"> ${T("common.docs.rustApi.title", {
    defaultValue: "Rust API Reference"
  })} </h2> <p class="text-sm text-muted-foreground"> ${T("common.docs.rustApi.description", {
    ...
    return renderTemplate`<section class="mt-12"${addAttribute(Section.toLowerCase().replace(/\s+/g, "-"), "id")}> <h2 class="mb-6 text-2xl font-semibold"> ${Section} </h2> <div class="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3"> ${Entries.map((Entry) => renderTemplate`<a${addAttribute(`/Doc/${Entry.id}`, "href")} class="StaccatoCard StaccatoBorderShimmer block border border-[var(--Border)] bg-white p-4 transition-colors hover:bg-[var(--Secondary)] focus:outline-2 focus:outline-offset-2 focus:outline-[var(--Primary)]"> <h3 class="font-medium"> ${Entry.data.title} </h3> ${Entry.data.description && renderTemplate`<p class="mt-1 text-xs text-muted-foreground"> ${Entry.data.description} </p>`} </a>`)} </div> </section>`;
  ...
  })} </p> <div class="flex flex-wrap gap-3"> ${Element.map((E) => renderTemplate`<a${addAttribute(`/Doc/${E.Name.toLowerCase()}`, "href")} class="inline-flex items-center gap-1 border border-[var(--Border)] bg-white px-3 py-1.5 text-sm transition-colors hover:bg-[var(--Secondary)] focus:outline-2 focus:outline-offset-2 focus:outline-[var(--Primary)]"> ${E.Name} </a>`)} </div> <div class="mt-4 border-t border-[var(--Border)] pt-4"> <a href="https://github.com/CodeEditorLand" target="_blank" rel="noopener noreferrer" class="text-sm font-medium text-[var(--Primary)] hover:underline"> ${T("common.docs.repositories.viewAll", {
  ...
  })} </h2> <div class="StaccatoCard StaccatoBorderShimmer border border-[var(--Border)] bg-white p-6"> <p class="mb-4 text-muted-foreground"> ${T("common.docs.workers.intro", {
    defaultValue: "Four Cloudflare Workers power the Land backend infrastructure:"
  })} </p> <ul class="space-y-3 text-sm"> <li> <a href="https://codeeditorland-auth.playform.workers.dev" target="_blank" rel="noopener noreferrer" class="font-semibold text-[var(--Primary)] hover:underline">Auth<span class="InlineSeparator">↗</span></a> <span class="text-muted-foreground"> ${" - "} ${T("common.docs.workers.auth", {
  ...
  })} </h2> <div class="StaccatoCard StaccatoBorderShimmer border border-[var(--Border)] bg-white p-6"> <p class="mb-4 text-muted-foreground"> ${T("common.docs.deployments.intro", {
    defaultValue: "Public website and documentation surfaces are deployed separately from the local editor runtime:"
  })} </p> <div class="grid grid-cols-1 gap-3 text-sm md:grid-cols-2"> <a href="https://editor.land" target="_blank" rel="noopener noreferrer" class="block border border-[var(--Border)] bg-white px-4 py-3 transition-colors hover:bg-[var(--Secondary)]"> <span class="font-semibold">${T("common.docs.deployments.website", {
    defaultValue: "Editor.Land"
  })}</span> <span class="InlineSeparator">↗</span> <p class="mt-1 text-xs text-muted-foreground"> ${T(
    "common.docs.deployments.website.description",
    {
      defaultValue: "Marketing website (Cloudflare Pages)"
    }
  )} </p> </a> <a href="https://Status.Editor.Land" target="_blank" rel="noopener noreferrer" class="block border border-[var(--Border)] bg-white px-4 py-3 transition-colors hover:bg-[var(--Secondary)]"> <span class="font-semibold">${T("common.docs.deployments.status", {
    defaultValue: "Status"
  })}</span> <span class="InlineSeparator">↗</span> <p class="mt-1 text-xs text-muted-foreground"> ${T(
  ...
  )} </p> </a> <a href="https://Rust.Documentation.Mountain.Editor.Land" target="_blank" rel="noopener noreferrer" class="block border border-[var(--Border)] bg-white px-4 py-3 transition-colors hover:bg-[var(--Secondary)]"> <span class="font-semibold">${T(
  ...
  )} </p> </a> <a href="https://Rust.Documentation.Rest.Editor.Land" target="_blank" rel="noopener noreferrer" class="block border border-[var(--Border)] bg-white px-4 py-3 transition-colors hover:bg-[var(--Secondary)]"> <span class="font-semibold">${T("common.docs.deployments.rustRest", {
  ...
  )} </p> </a> <a href="https://Rust.Documentation.Maintain.Editor.Land" target="_blank" rel="noopener noreferrer" class="block border border-[var(--Border)] bg-white px-4 py-3 transition-colors hover:bg-[var(--Secondary)]"> <span class="font-semibold">${T(
  ...
  )} </p> </a> <a href="https://Rust.Documentation.Common.Editor.Land" target="_blank" rel="noopener noreferrer" class="block border border-[var(--Border)] bg-white px-4 py-3 transition-colors hover:bg-[var(--Secondary)]"> <span class="font-semibold">${T(
  ...
  )} </p> </a> <a href="https://Rust.Documentation.Echo.Editor.Land" target="_blank" rel="noopener noreferrer" class="block border border-[var(--Border)] bg-white px-4 py-3 transition-colors hover:bg-[var(--Secondary)]"> <span class="font-semibold">${T("common.docs.deployments.rustEcho", {
  ...
  )} </p> </a> <a href="https://Rust.Documentation.Air.Editor.Land" target="_blank" rel="noopener noreferrer" class="block border border-[var(--Border)] bg-white px-4 py-3 transition-colors hover:bg-[var(--Secondary)]"> <span class="font-semibold">${T("common.docs.deployments.rustAir", {
  ...
  )} </p> </a> <a href="https://Rust.Documentation.SideCar.Editor.Land" target="_blank" rel="noopener noreferrer" class="block border border-[var(--Border)] bg-white px-4 py-3 transition-colors hover:bg-[var(--Secondary)]"> <span class="font-semibold">${T(
  ...
  )} </p> </a> <a href="https://Rust.Documentation.Land.Editor.Land" target="_blank" rel="noopener noreferrer" class="block border border-[var(--Border)] bg-white px-4 py-3 transition-colors hover:bg-[var(--Secondary)]"> <span class="font-semibold">${T("common.docs.deployments.rustLand", {
  ...
  )} </p> </a> <a href="https://Knowledge.Editor.Land" target="_blank" rel="noopener noreferrer" class="block border border-[var(--Border)] bg-white px-4 py-3 transition-colors hover:bg-[var(--Secondary)]"> <span class="font-semibold">${T("common.docs.deployments.knowledge", {
  ...
  )} </p> </a> <a href="/Doc/Rust" class="block border border-[var(--Border)] bg-white px-4 py-3 transition-colors hover:bg-[var(--Secondary)]"> <span class="font-semibold">${T(
  ...
  )} </p> </a> </div> </div> </section> <!-- Back to top --> <div class="mt-12 border-t pt-8"> <a href="#top" class="StaccatoButton text-sm text-primary hover:underline">${T("common.button.backToTop", {
