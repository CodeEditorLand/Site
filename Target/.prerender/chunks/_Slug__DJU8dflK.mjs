...
import { getCollection, render as renderEntry } from './_astro_content_Do7_53PI.mjs';
...
const $$Slug = createComponent(async ($$result, $$props, $$slots) => {
  ...
  return renderTemplate`${renderComponent($$result, "Base", $$Base, { "Title": MetaTitle, "Description": MetaDescription, "Url": `https://editor.land/Doc/${Entry.id}`, "lang": "en" }, { "default": async ($$result2) => renderTemplate` ${renderComponent($$result2, "Header", Header, { "client:load": true, "client:component-hydration": "load", "client:component-path": "/Volumes/CORSAIR/Developer/macOS/Application/CodeEditorLand/WebSite/Source/Component/Layout/Header", "client:component-export": "Header" })} ${maybeRenderHead()}<div class="container mx-auto max-w-6xl px-4 py-16"> <div class="flex gap-8"> <!-- Sidebar --> <aside class="hidden w-56 shrink-0 lg:block"> <div class="sticky top-8"> <p class="mb-3 text-xs font-semibold uppercase tracking-wider text-muted-foreground"> ${T("sidebar.title", {
    ...
    ns: "doc",
    defaultValue: "Documentation"
  })}</a> </li> <li class="mx-2">/</li> <li> <span class="font-medium text-foreground">${Entry.data.title}</span> </li> </ol> </nav> <!-- Article header --> <header class="mb-8"> <p class="mb-2 text-xs font-semibold uppercase tracking-wider text-muted-foreground"> ${Entry.data.section} </p> <h1 class="mb-3 text-4xl font-bold tracking-tight"> ${Entry.data.title} </h1> <p class="text-lg text-muted-foreground"> ${Entry.data.description} </p> </header> <!-- Doc content --> <div class="prose prose-neutral max-w-none border-t border-[var(--ColorBorder)] pt-8"> ${renderComponent($$result2, "Content", Content, {})} </div> <!-- Prev / Next navigation --> ${(PrevEntry || NextEntry) && renderTemplate`<div class="mt-12 flex justify-between border-t border-[var(--ColorBorder)] pt-8 text-sm"> ${PrevEntry ? renderTemplate`<a${addAttribute(`/Doc/${PrevEntry.id}`, "href")} class="StaccatoButton text-primary hover:underline"> ${PrevEntry.data.title} <span class="InlineSeparator">←</span> </a>` : renderTemplate`<span></span>`} ${NextEntry && renderTemplate`<a${addAttribute(`/Doc/${NextEntry.id}`, "href")} class="StaccatoButton text-primary hover:underline"> ${NextEntry.data.title} <span class="InlineSeparator">→</span> </a>`} </div>`} </main> </div> </div> ` })}`;
