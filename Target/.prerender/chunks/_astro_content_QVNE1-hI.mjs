...
export { z };
import { e as renderElement, g as generateCspDigest, s as spreadAttributes, u as unescapeHTML, a as renderTemplate, A as AstroError, L as LiveContentConfigError, f as AstroUserError, R as RenderUndefinedEntryError, h as UnknownContentCollectionError, G as GetEntryDeprecationError, i as createHeadAndContent, r as renderComponent } from './prerender_CyJMqgoM.mjs';
...
import { V as VALID_INPUT_FORMATS } from './OpenGraph_Ibpu4DcJ.mjs';
import 'piccolore';
...
function createGetCollection({
  ...
  return async function getCollection(collection, filter) {
    ...
    if (store.hasCollection(collection)) {
      const { default: imageAssetMap } = await import('./OpenGraph_Ibpu4DcJ.mjs').then(n => n.d);
      const result = [];
...
async function updateImageReferencesInBody(html, fileName) {
  ...
  const { getImage } = await import('./_virtual_astro_get-image_D4N97aw8.mjs');
...
async function renderEntry(entry) {
  ...
  if (entry.deferredRender) {
    try {
      const { default: contentModules } = await import('./Turbulence_MCudHMuZ.mjs').then(n => n.d);
