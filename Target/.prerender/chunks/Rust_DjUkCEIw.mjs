...
</p> </header> <!-- Crate selector cards --> <div class="mb-6 grid grid-cols-2 gap-3 md:grid-cols-5" role="group" aria-label="Rust crate documentation sets"> ${RustDocSite.map((Site) => renderTemplate`<a${addAttribute(Site.Url, "href")} target="_blank" rel="noopener noreferrer" class="StaccatoCard StaccatoBorderShimmer block border border-[var(--Border)] bg-white p-4 transition-colors hover:border-[var(--Primary)] focus:outline-2 focus:outline-offset-2 focus:outline-[var(--Primary)]"> <p class="font-medium text-[var(--Primary)]"> ${Site.Label} <span class="InlineSeparator">↗</span> </p> <p class="mt-1 text-xs text-muted-foreground"> ${Site.Description} </p> </a>`)} </div> <!-- Iframe embed with switcher --> <div class="StaccatoCard StaccatoBorderShimmer overflow-hidden border border-[var(--Border)] bg-white"> <!-- Tab bar --> <div class="flex flex-wrap items-center gap-1 border-b border-[var(--Border)] px-4 py-2" role="tablist" aria-label="Rust crate group"> ${RustDocSite.map((Site, Index) => renderTemplate`<button type="button" role="tab"${addAttribute(Index === 0 ? "true" : "false", "aria-selected")} aria-controls="RustDocFrame"${addAttribute(Site.Id, "data-id")}${addAttribute(Site.Url, "data-rustdoc-url")}${addAttribute(`RustDocTab border px-3 py-1 text-xs font-medium transition-colors focus:outline-2 focus:outline-offset-2 focus:outline-[var(--Primary)] ${Index === 0 ? "border-[var(--Primary)] bg-[var(--Primary)] text-white" : "border-[var(--Border)] bg-white text-muted-foreground hover:bg-[var(--Secondary)]"}`, "class")}> ${Site.Label} </button>`)} </div> <iframe id="RustDocFrame"${addAttribute(RustDocSite[0].Url, "src")} title="Rust API Documentation" class="h-[70vh] w-full border-0" loading="lazy" sandbox="allow-scripts allow-same-origin allow-popups allow-top-navigation-by-user-activation"></iframe> </div> <!-- Back to docs --> <div class="mt-8 border-t pt-6"> <a href="/Doc" class="StaccatoButton text-sm text-primary hover:underline"> ${T("common.docs.pageTitle", { defaultValue: "Documentation" })} <span class="InlineSeparator">←</span> </a> </div> </div> ` })} ${renderScript($$result, "/Volumes/CORSAIR/Developer/macOS/Application/CodeEditorLand/WebSite/Source/pages/Doc/Rust.astro?astro&type=script&index=0&lang.ts")}`;
