...
export { z };
import { x as renderElement, s as generateCspDigest, C as spreadAttributes, D as unescapeHTML, B as renderTemplate, A as AstroError, L as LiveContentConfigError, a as AstroUserError, k as RenderUndefinedEntryError, U as UnknownContentCollectionError, G as GetEntryDeprecationError, p as createHeadAndContent, w as renderComponent } from './prerender_AmZqEYF9.mjs';
...
import { V as VALID_INPUT_FORMATS } from './PageMetadata_DYW59jf6.mjs';
import 'piccolore';
...
function createGetCollection({
  ...
  return async function getCollection(collection, filter) {
    ...
    if (store.hasCollection(collection)) {
      const { default: imageAssetMap } = await import('./PageMetadata_DYW59jf6.mjs').then(n => n.i);
      const result = [];
...
async function updateImageReferencesInBody(html, fileName) {
  ...
  const { getImage } = await import('./_virtual_astro_get-image_CtqE960_.mjs');
...
async function renderEntry(entry) {
  ...
  if (entry.deferredRender) {
    try {
      const { default: contentModules } = await import('./OpenGraph_BjPLYAEW.mjs').then(n => n.d);
