...
## 0. Current Deployment State (read first)

The Staccato noise engine is **permanently neutralized for position transforms
and parallax**. An always-on block at the top of
`Source/Function/Noise/Stylesheet.css` zeros every translate/rotate/scale
transform generated by the noise system. Named card classes (`.FeatureCard`,
`.PricingCard`, `.TestimonialCard`, `.TransparencyCard`, `.MasonryCard`) also
force `transform: none`. The hero orbital animation is disabled.

**What is still active:**

- Tier 3 opacity, color-shift, border-weight, and glow channels (no position
  change)
- `StaccatoVisible` fade-in on intersection
- Hover/focus settle behavior (parent interaction surface → children settle)
- `prefers-reduced-motion` handling

**What is zeroed:**

- All Tier 1 seed-based position transforms (`StaccatoTranslate`, `StaccatoCard`
  scatter/rotate/scale, `StaccatoRotate`, `StaccatoJitter`)
- Tier 4 scroll-linked parallax (`StaccatoParallaxNear/Mid/Far`)
- Hero `StaccatoFloat` / `StaccatoHeroButton` live-float

Do not reintroduce noise-driven position transforms. New sections must be
static and grid-aligned. The visual hierarchy comes from card borders, accent
colors, and typography - not from motion.

---

## 1. Definition

“Staccato” is the site’s personality layer: a deterministic noise signal that
drives opacity, color, border, and glow channels. Position transforms from this
system are permanently zeroed (see §0).
