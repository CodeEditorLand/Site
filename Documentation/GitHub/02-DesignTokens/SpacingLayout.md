...
`--Spacing: 0.25rem` (4 px). Tailwind’s default spacing scale is used
throughout.
...
- Gap class may be overridden by `StaccatoMorphGap` (note: actual gap morphing
  is disabled to prevent CLS; class is present for compatibility).
...
- Between major sections: `mb-24` on wrapper / `mx-auto max-w-2xl text-center`
  intro.
